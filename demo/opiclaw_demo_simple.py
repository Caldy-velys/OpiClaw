...
def demonstrate_language_guidance():
    ...
    print(
        f"Testing {len(scenes)} scene types with {len(prompts)} prompt variations...\n"
    )
    ...
    for scene_name, points in scenes.items():
        ...
        for prompt in prompts:
            ...
            print(
                f"  📝 '{prompt_str:20s}' -> Uncertainty: {uncertainty:.3f}, Confidence: {confidence:.3f}"
            )
...
def demonstrate_uncertainty_analysis():
    ...
    for scenario_name, points in scenarios.items():
        ...
        print(
            f"  Prediction Entropy: {(-prob * torch.log(prob + 1e-8)).sum(0).mean():.3f}"
        )
...
def demonstrate_architecture_components():
    ...
    components = {
        ...
        "Decoder": sum(
            p.numel() for p in [*model.dec1.parameters(), *model.dec2.parameters()]
        ),
...
def create_visualization():
    ...
    scenarios = [
        (
            ...
            generate_marine_scene(1000, "hydrothermal_field"),
            "find hydrothermal_vent",
        ),
        (
            "Debris Field",
            generate_marine_scene(1000, "debris_field"),
            "detect debris wreck",
        ),
        (
            "Mixed Environment",
    ...
    plt.suptitle(
        "OpiClaw Enhanced: ConvViT + LGRS Fusion for Deep-Sea Segmentation",
        fontsize=16,
        fontweight="bold",
    )
    plt.tight_layout()
