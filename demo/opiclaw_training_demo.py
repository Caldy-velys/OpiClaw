...
from opiclaw_upgrade import (
    ...
    evidential_loss, MarineLGRSFusion
)
...
class MarineDataset(Dataset):
    ...
    def _generate_instance_gt(self, bev, scene_type):
        ...
        if scene_type in ["hydrothermal_field", "debris_field"]:
            ...
            for i, center in enumerate(centers):
                r, c = center
                instance_gt[max(0, r-3):min(H, r+4), max(0, c-3):min(W, c+4)] = i + 1
    ...
    def _generate_prompt(self, scene_type):
        ...
        prompts = {
            "hydrothermal_field": ["find hydrothermal_vent", "detect vent", "map hydrothermal_vent"],
...
def train_epoch(model, dataloader, optimizer, device):
    ...
    for batch in tqdm(dataloader, desc="Training"):
        ...
        bev = torch.cat([sample['bev'] for sample in batch], dim=0).to(device)  # Proper batching
        semantic_gt = torch.stack([sample['semantic_gt'] for sample in batch], dim=0).to(device)
        ...
        center_loss = F.mse_loss(outputs['instance_center'], torch.zeros_like(outputs['instance_center']))
...
def visualize_predictions(model, dataset, device, num_samples=4):
    ...
    for i in range(num_samples):
        ...
        pred_with_prompt = outputs_with_prompt['semantic_prob'].argmax(1).squeeze().cpu()
        uncertainty_no_prompt = outputs_no_prompt['semantic_uncertainty'].squeeze().cpu()
        uncertainty_with_prompt = outputs_with_prompt['semantic_uncertainty'].squeeze().cpu()
...
def main():
    ...
    # Dataset
    train_dataset = MarineDataset(num_samples=500, scene_types=["hydrothermal_field", "debris_field", "flat_seafloor"])
    val_dataset = MarineDataset(num_samples=100, scene_types=["hydrothermal_field", "debris_field", "flat_seafloor"])
    
    train_loader = DataLoader(train_dataset, batch_size=4, shuffle=True, collate_fn=marine_collate_fn)
    val_loader = DataLoader(val_dataset, batch_size=4, shuffle=False, collate_fn=marine_collate_fn)
    ...
    for epoch in range(5):  # Short demo training
        ...
        print(f"Train Loss: {train_loss:.4f}, Val Acc: {val_acc:.4f}, Val Uncertainty: {val_unc:.4f}")
    ...
    for prompt in marine_prompts:
        with torch.no_grad():
            ...
            pred_entropy = (-outputs['semantic_prob'] * torch.log(outputs['semantic_prob'] + 1e-8)).sum(1).mean().item()
        
        print(f"Prompt: '{prompt:20s}' -> Uncertainty: {uncertainty:.3f}, Entropy: {pred_entropy:.3f}")
